---
title: ' Course 5: Reproducible research, week 5: PA1_template'
...
First the working directory is set, then the data is downloaded and loaded and finally the necessary libraries are loaded. 
After the data is loaded, the data is saved in a variable and the necessary transformations are carried out. 
```{r}
...
A histogram of the total number of steps taken each day is created using the loaded data. 
...
The mean and median are calculated using the total steps taken per day. The NA values are removed when calculating the mean and median.
...
First the average number of steps taken for each interval is calculated. This is done by using the data.table package and calculated the the mean steps for each interval across al days. 
...
Using the average number of steps per internval, a time series plot is created
...
First create a new temporary data set, where the whole data set is reloaded
...
Then the mean for each interval is recalculated and used to fill up the missing values

```{r, echo = TRUE}
mean_steps_perinterval <- aggregate(steps~interval, tmp_data,mean)
```

A new dataset is then created where the missing NA values are filled in with the calculated mean per day
```{r, echo = TRUE}
...
```
These new median and mean have a different value than the previous ones. The impact of imputing data can therefore be considered large.
...
A new factor variable with two levels. This variable is joined into the existing data variable
```{r, echo = TRUE}
...
The data set of the 5 minute interval data averaged across the weekdays and weekends is created for the plot using the data.table package
```{r, echo = TRUE}
tmp_total_data <- data.table(total_data)
new_average_steps_perinterval <- tmp_total_data[, list(steps = mean(steps)), by = c("day","interval")]
```

A time series plot is created using the ggplot function, where the plots for weekdays and weekend are shown on top of each other
```{r, echo = TRUE, fig.height= 6, fig.width=5}
